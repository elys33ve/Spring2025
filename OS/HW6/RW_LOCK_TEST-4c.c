...
// I'm working with a "fill array" of 1000 slots. Feel free to experiment with this number
#define  ARRAY_SIZE  1000

// standard mutex.  
// When you test your read-write lock, uncomment the line after it. 
// BE SURE YOUR CODE WORKS WITH THE RW_lock_t array_lock declaration below.
// FOLLOW THE API SPEC IN THE HOMEWORK OR WE WON'T BE ABLE TO GRADE YOUR ASSIGNMENT
...
// big_array[] - array being filled and beging (concurrently or in parallel) checked by the other threads.  
int       big_array[ARRAY_SIZE];
// Data_Insert_Count contains the count of items already "read" into the big_array[].
// As the filler thread runs, it will increment appropriately. reader threads use it to 
// monitor how many values have been read, using that number to compute its "checksum"
int       Data_Insert_Count = 0;

// *put_item_in_array() associated with the "fill the array" thread. 
// Note that it iterates until the array have been given ARRAY_SIZE values.
...
{  int c;                                // I'll use this for generic counts
   ...
   // First, zero out the contents of the big_array.
   for (c=0; c < ARRAY_SIZE; c++)
      big_array[c] = 0;

   // Now, launch the threads
   // I'm launching exactly ONE filler thread and <check_thread_count> reader "checksum"
